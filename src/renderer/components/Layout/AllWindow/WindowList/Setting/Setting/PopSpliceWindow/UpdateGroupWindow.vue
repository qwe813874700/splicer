  ...
  <div class="add-group flex-box">
    <div class="group-left flex-1 mg-r" @mousewheel.prevent="scaleBack" @click="clearPresetSelect">
      <div class="pop-window-back" :style="backTransformObj">
        <div class="pop-window-item"
          ...
          <div v-if="item.resWidth / 1920 > 0.7">
    ...
    <div class="group-right">
      ...
      <div class="container-footer">
        <el-button size="mini" type="primary" @click="updateGroupList">确认</el-button>
      </div>
    </div>
    <setting-resolution 
...
export default {
  ...
  methods: {
    _initWindowData () {
      this.allInfo = JSON.parse(JSON.stringify(this.currTableList))
      console.log(this.allInfo)
      this.setPresetGroupList(this.windowItemList.list.filter(item => this.allInfo.groupIdList.indexOf(item.id) > -1))
    },
    /**
    ...
    startSelectGroupItem (e, item) { // 通过拖动选择范围内的屏幕
      ...
      document.onmouseup = (event) => { // 鼠标松开, 幕布消失,计算在当前选择范围内的窗口
        ...
        for (let i = 0; i < selectItem.length; i++) { // 判断当前范围内是否有已选中的窗口, 若有则不选中
          ...
          if (this.allInfo.groupIdList.indexOf(element.id) > -1) { // 若选择的是原来分组的, 则直接跳过  默认可选择
            continue
          }
    ...
    updateGroupList () {
      ...
      const data = {
        guihead: 'setSplicingGroup',
        groupIdx: this.allInfo.id,
        groupName: this.allInfo.groupName,
        ...
        screenH_Edge: this.allInfo.screenH_Edge,
        screenV_Edge: this.allInfo.screenV_Edge,
        rezIdx: this.allInfo.rezIdx,
        rateIdx: this.allInfo.rateIdx,
        screenSetting: this.currResRushObj,
        splicingModule: this.allInfo.splicingModule
      }
      ...
      this.setGroupList({
        type: 'update',
        index: this.groupList.findIndex(item => item.groupIdx === data.groupIdx),
        value: data
      })
    ...
    setSelectItem (item, groupId) { // 选择单个屏幕
      if (typeof groupId !== 'number' || groupId === this.allInfo.id) {
